<!-- eslint-disable vue/multi-word-component-names -->
<template>
  <footer>
    <v-container>
      <v-row align="center" justify="center">
        <v-col cols="12" lg="4">
          <div class="d-flex flex-column justify-center align-center">
            <!-- <div class="qr">
              <img src="/qrcode.png" alt="logo" />
            </div> -->
            <div class="social">
              <a href="https://web.facebook.com/Sahel1alex/" target="_blank">
                <v-icon class="icon">mdi-facebook</v-icon>
              </a>
              <a href="https://www.tiktok.com/@sahel_alex" target="_blank">
                <div class="icon">
                  <svg
                    width="20px"
                    height="20px"
                    viewBox="0 0 24 24"
                    role="img"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <title>TikTok icon</title>
                    <path
                      d="M12.53.02C13.84 0 15.14.01 16.44 0c.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"
                    />
                  </svg>
                </div>
              </a>
              <a href="https://www.instagram.com/sahel_alex_resturant" target="_blank">
                <v-icon class="icon">mdi-instagram</v-icon>
              </a>
            </div>
          </div>
        </v-col>
        <v-col cols="12" lg="4">
          <div class="image">
            <img src="/logo.png" alt="logo" />
          </div>
        </v-col>
        <v-col cols="12" lg="4">
          <div class="contact">
            <div>
              <v-icon class="icon">mdi-phone</v-icon>
              <a href="tel:0593657309">0593657309</a>
            </div>
            <div>
              <v-icon class="icon">mdi-phone</v-icon>
              <a href="tel:0593657306">0593657306</a>
            </div>
            <div>
              <v-icon class="icon">mdi-email</v-icon>
              <a href="tel:0593657306">email@example.com</a>
            </div>
            <div class="address d-flex">
              <v-icon class="icon">mdi-map-marker</v-icon>
              <span>
                الرياض - حي الوشام
                <br />تقاطع شارع عبد العزيز بن محمد بن سعود مع شارع الخزان
                <br />
              </span>
            </div>
          </div>
        </v-col>
      </v-row>
    </v-container>
  </footer>
</template>

<script>
export default {};
</script>

<style></style>
